...
path = 'C:\Users\Elias\Desktop\fMRI\log_data\xls'
file = 'CRLfmri_999.xls'

%% open file
[numbers, strings, raw] = xlsread('C:\Users\Elias\Desktop\fMRI\log_data\xls\CRLfmri_999.xls');

%Note, that 'strings' and 'raw' contain header row, but 'numbers' does not.
%Therefore, when accessing 'numbers', row indices retrieved from 'raw' are
%subtracted by 1 

%onset arrays for selection, baseline, game_display and game_feedback for
%each block
sel1 = []; sel2 = []; sel3 = [];
base1 = []; base2 = []; base3 = [];
gameP_disp1 = []; gameP_fb1 = []; gameP_disp2 = []; gameP_fb2 = []; gameP_disp3 = []; gameP_fb3 = [];
gameL_disp1 = []; gameL_fb1 = []; gameL_disp2 = []; gameL_fb2 = []; gameL_disp3 = []; gameL_fb3 = [];
gameT_disp1 = []; gameT_fb1 = []; gameT_disp2 = []; gameT_fb2 = []; gameT_disp3 = []; gameT_fb3 = []; 
...
%indices for trash regressor
instrS_ind = find(ismember(strings(1,:),'time_Selection'));
instrG_ind = find(ismember(strings(1,:),'time_Game_start'));
fb_ind = find(ismember(strings(1,:),'time_Block_FB'));

%grab rows according to phase
phase = strings(2:end,phase_ind);       %array of experiment phase for each trial
sel_row = find(ismember(raw(:,phase_ind),'selection'));  %array of rows in selection phase 
%only take first rows of one active part
sel_row = sel_row(1:8:size(sel_row,1));

game_row = find(ismember(raw(:,phase_ind),'game'));           %array of rows in game phase

%grab rows according to blocks
blocks = numbers(:,blocknr_ind); %% NOT NEEDED!?
...
%grab rows of each target_dimension in game
target_dimension = strings(:,target_ind); % NOT NEEDED!?
...
%grab rows of baseline in selection
base = numbers(:,base_ind);
base_row = find(ismember(numbers(:,base_ind),15000))+1;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%fill arrays for display of stimuli of each block
%selection, time_show_stim

%fill arrays for block 1: show stim selection (sel1), %baseline_selection(base1) and response time sel (resp_sel1)
...
%fill arrays for block 2: show stim selection (sel2), %baseline_selection(base2) and response time sel (resp_sel2)
...
%fill arrays for block 3: show stim selection (sel3), %baseline_selection(base3) and response time sel (resp_sel3)
...
%fill arrays for each target_dimension in separate blocks
...
L2 = intersect(L,block2); L2p = intersect(L1,positive); L2n = intersect(L1,negative);
L3 = intersect(L,block3); L3p = intersect(L1,positive); L3n = intersect(L1,negative);
...
T1 = intersect(T,block1); T1p = intersect(L1,positive); T1n = intersect(L1,negative);
T2 = intersect(T,block2); T2p = intersect(L1,positive); T2n = intersect(L1,negative);
T3 = intersect(T,block3); T3p = intersect(L1,positive); T3n = intersect(L1,negative);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DURATIONS
%durations for show stim in selection --> RTs of individual trial, else 1000ms
...
dur_gameP_disp1 = numbers(P1-1,resp_ind)/1000;
dur_gameP_disp2 = numbers(P2-1,resp_ind)/1000;
dur_gameP_disp3 = numbers(P3-1,resp_ind)/1000;
dur_gameL_disp1 = numbers(L1-1,resp_ind)/1000;
dur_gameL_disp2 = numbers(L2-1,resp_ind)/1000;
dur_gameL_disp3 = numbers(L3-1,resp_ind)/1000;
dur_gameT_disp1 = numbers(T1-1,resp_ind)/1000;
dur_gameT_disp2 = numbers(T2-1,resp_ind)/1000;
dur_gameT_disp3 = numbers(T3-1,resp_ind)/1000;
%durations for feedback screen (700ms)
dur_gameP_fbp1 = 0.7.* ones(size(gameP_fbp1)); dur_gameP_fbn1 = 0.7.* ones(size(gameP_fbn1)); 
dur_gameP_fbp2 = 0.7.* ones(size(gameP_fbp2)); dur_gameP_fbn2 = 0.7.* ones(size(gameP_fbn2)); 
dur_gameP_fbp3 = 0.7.* ones(size(gameP_fbp3)); dur_gameP_fbn3 = 0.7.* ones(size(gameP_fbn3)); 
dur_gameL_fbp1 = 0.7.* ones(size(gameL_fbp1)); dur_gameL_fbn1 = 0.7.* ones(size(gameL_fbn1));
dur_gameL_fbp2 = 0.7.* ones(size(gameL_fbp2)); dur_gameL_fbn2 = 0.7.* ones(size(gameL_fbn2));
dur_gameL_fbp3 = 0.7.* ones(size(gameL_fbp3)); dur_gameL_fbn3 = 0.7.* ones(size(gameL_fbn3));
dur_gameT_fbp1 = 0.7.* ones(size(gameT_fbp1)); dur_gameT_fbn1 = 0.7.* ones(size(gameT_fbn1));
dur_gameT_fbp2 = 0.7.* ones(size(gameT_fbp2)); dur_gameT_fbn2 = 0.7.* ones(size(gameT_fbn2));
dur_gameT_fbp3 = 0.7.* ones(size(gameT_fbp3)); dur_gameT_fbn3 = 0.7.* ones(size(gameT_fbn3));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%parametric modulator (3 trials) % correct

acc_ind = find(ismember(strings(1,:),'Acc+')); %% ACC Index --> Change with new data files!!!!
P1cor = numbers(P1-1,correct_ind); %acc for block1 game1
P2cor = numbers(P2-1,correct_ind); %acc for block1 game2
P3cor = numbers(P3-1,correct_ind);
L1cor = numbers(L1-1,correct_ind);
L2cor = numbers(L2-1,correct_ind);
L3cor = numbers(L3-1,correct_ind);
T1cor = numbers(T1-1,correct_ind);
T2cor = numbers(T2-1,correct_ind);
T3cor = numbers(T3-1,correct_ind);

b1cor = {P1cor L1cor T1cor};
b2cor = {P2cor L2cor T2cor};
b3cor = {P3cor L3cor T3cor};
...
%%TRASH REGRESSOR
%%Get first trial for each game for game_instructions
...
%contains onsets for game instructions for each block
trashgame1 = game_start(:,1:3);
trashgame2 = game_start(:,4:6);
trashgame3 = game_start(:,7:9);

%%get first trials of each selection for selection_instructions
sel_begin = sel_row(1:4:end,1);

%contains rows of first trial in selection
...
%contains onset for selection instructions for each block
trashsel1 = sel_start(:,1:3);
trashsel2 = sel_start(:,4:6);
trashsel3 = sel_start(:,7:9);

%FEEDBACK SCREENS
%Get trials of Block_FB (erster Trial eines neuen Blocks oder eins nach letztem Spiel pro Block)
%(Erstes Mal am Ende des 1. Blocks)
%LETZTES BLOCK FB FEHLT HIER NOCH!!!!!!!!!!!!!!!!!
...
%%%Create TRASH PMOD
trashnames = {'Instructions','Block_FB'}
...
%%Uncomment when Block FB at end works!!    field2 = trashnames{2}; value2 = trashmod1{2};
trashreg1 = struct(field1,value1); %change to: trashreg1 = struct(field1,value1,field2,value2)
%block2
field1 = trashnames{1}; value1 = trashmod2{1};
%%Uncomment when Block FB at end works!!     %field2 = trashnames{2}; value2 = trashmod2{2};
trashreg2 = struct(field1,value1); %change to: trashreg2 = struct(field1,value1,field2,value2
%block3
field1 = trashnames{1}; value1 = trashmod3{1};
%%Uncomment when Block FB at end works!!     %field2 = trashnames{2}; value2 = trashmod3{2};
...
%block1
names = {'gameP_choice','gameL_choice','gameT_choice','gameP_fb_positive','gameP_fb_negative','gameL_fb_positive','gameL_fb_negative','gameT_fb_positive','gameT_fb_negative','sel','base_sel','Instructions'}; %add Block_FB at end
onsets = {gameP_disp1,gameL_disp1,gameT_disp1,gameP_fbp1,gameP_fbn1,gameL_fbp1,gameL_fbn1,gameT_fbp1,gameT_fbn1,sel1,base1,trash1'}; %add trashfb1 at end
durations = {dur_gameP_disp1,dur_gameL_disp1,dur_gameT_disp1,dur_gameP_fbp1,dur_gameP_fbn1,dur_gameL_fbp1,dur_gameL_fbn1,dur_gameT_fbp1,dur_gameT_fbn1,dur_sel1,dur_base1,dur_Instr}; %add dur_FB at end
pmod = pmod1;
save(['999_block1'],'names','onsets','durations','pmod')

%block2
names = {'gameP_choice','gameL_choice','gameT_choice','gameP_fb_positive','gameP_fb_negative','gameL_fb_positive','gameL_fb_negative','gameT_fb_positive','gameT_fb_negative','sel','base_sel','Instructions'}; %add Block_FB at end
onsets = {gameP_disp2,gameL_disp2,gameT_disp2,gameP_fbp2,gameP_fbn2,gameL_fbp2,gameL_fbn2,gameT_fbp2,gameT_fbn2,sel2,base2,trash2'}; %add trashfb2 at end
durations = {dur_gameP_disp2,dur_gameL_disp2,dur_gameT_disp2,dur_gameP_fbp2,dur_gameP_fbn2,dur_gameL_fbp2,dur_gameL_fbn2,dur_gameT_fbp2,dur_gameT_fbn2,dur_sel2,dur_base2,dur_Instr}; %add dur_FB at end!
pmod = pmod2;
save(['999_block2'],'names','onsets','durations','pmod')

%block3
%ADD BLOCK FB!!!
names = {'gameP_choice','gameL_choice','gameT_choice','gameP_fb_positive','gameP_fb_negative','gameL_fb_positive','gameL_fb_negative','gameT_fb_positive','gameT_fb_negative','sel','base_sel','Instructions'};
onsets = {gameP_disp3,gameL_disp3,gameT_disp3,gameP_fbp3,gameP_fbn3,gameL_fbp3,gameL_fbn3,gameT_fbp3,gameT_fbn3,sel3,base3,trash3'}; %add blockFB?
durations = {dur_gameP_disp3,dur_gameL_disp3,dur_gameT_disp3,dur_gameP_fbp3,dur_gameP_fbn3,dur_gameL_fbp3,dur_gameL_fbn3,dur_gameT_fbp3,dur_gameT_fbn3,dur_sel3,dur_base3,dur_Instr}; %add dur_FB
pmod = pmod3;
save(['999_block3'],'names','onsets','durations','pmod')
