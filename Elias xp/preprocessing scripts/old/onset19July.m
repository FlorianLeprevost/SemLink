...
%%ONSETS of congruent v incongruent feedback
%feedback onsets
con_fb1_ons = (numbers(cb1-1,game_fb_ind)-trigger1_delay)/1000;
con_fb2_ons = (numbers(cb2-1,game_fb_ind)-trigger2_delay)/1000;
con_fb3_ons = (numbers(cb3-1,game_fb_ind)-trigger3_delay)/1000;
icon_fb1_ons = (numbers(icb1-1,game_fb_ind)-trigger1_delay)/1000;
icon_fb2_ons = (numbers(icb2-1,game_fb_ind)-trigger2_delay)/1000;
icon_fb3_ons = (numbers(icb3-1,game_fb_ind)-trigger3_delay)/1000;
 
%feedback durations
con_fb1_dur = 0.5*ones(size(con_fb1_ons));
con_fb2_dur = 0.5*ones(size(con_fb2_ons));
con_fb3_dur = 0.5*ones(size(con_fb3_ons));
icon_fb1_dur = 0.5*ones(size(icon_fb1_ons));
icon_fb2_dur = 0.5*ones(size(icon_fb2_ons));
icon_fb3_dur = 0.5*ones(size(icon_fb3_ons));
...
%% PMOD1
% %correct of 3 trials
P1cor = numbers(P1-1,acc_ind); P2cor = numbers(P2-1,acc_ind); P3cor = numbers(P3-1,acc_ind);
L1cor = numbers(L1-1,acc_ind); L2cor = numbers(L2-1,acc_ind); L3cor = numbers(L3-1,acc_ind);
T1cor = numbers(T1-1,acc_ind); T2cor = numbers(T2-1,acc_ind); T3cor = numbers(T3-1,acc_ind);
...
%% PMOD2 FOR CONGRUENT V INCONGRUENT GAMES
...
pmod2_b1(1).name{1} = 'CON%correct';     pmod2_b1(1).name{2} = 'ICON%correct';
pmod2_b1(1).param{1} = pmod2_1{1}'; pmod2_b1(1).param{2} = pmod2_1{2}';
pmod2_b1(1).poly{1} = ones(size(pmod2_b1(1).param{1})); pmod2_b1(1).poly{2} = ones(size(pmod2_b1(1).param{2})); 

%block2 structure
pmod2_b2 = struct('name',{},'param',{},'poly',{}); 
pmod2_b2(1).name{1} = 'CON%correct';     pmod2_b2(1).name{2} = 'ICON%correct';
pmod2_b2(1).param{1} = pmod2_2{1}'; pmod2_b2(1).param{2} = pmod2_2{2}';
pmod2_b2(1).poly{1} = ones(size(pmod2_b2(1).param{1})); pmod2_b2(1).poly{2} = ones(size(pmod2_b2(1).param{2})); 

%block3 structure
pmod2_b3 = struct('name',{},'param',{},'poly',{}); 
pmod2_b3(1).name{1} = 'CON%correct';     pmod2_b3(1).name{2} = 'ICON%correct';
pmod2_b3(1).param{1} = pmod2_3{1}'; pmod2_b3(1).param{2} = pmod2_3{2}';
pmod2_b3(1).poly{1} = ones(size(pmod2_b3(1).param{1})); pmod2_b3(1).poly{2} = ones(size(pmod2_b3(1).param{2}));

%equal matrix sizes for pmod2 (first cell 20, second 40)
for k = 1:2
    if k == 1;
        if size(pmod2_b1.param{k},1) < 20
            s = size(pmod2_b1.param{k},1);
            diff = 20 - s;
            pmod2_b1.param{k}(end+diff) = 0;
            pmod2_b1.poly{k}(end+diff) = 0;
        end
    elseif k > 1;
        if size(pmod2_b1.param{k},1) < 40
            s = size(pmod2_b1.param{k},1);
            diff = 40 - s;
            pmod2_b1.param{k}(end+diff) = 0;
            pmod2_b1.poly{k}(end+diff) = 0;
...
for k = 1:2
    if k == 1;
        if size(pmod2_b2.param{k},1) < 20
            s = size(pmod2_b2.param{k},1);
            diff = 20 - s;
            pmod2_b2.param{k}(end+diff) = 0;
            pmod2_b2.poly{k}(end+diff) = 0;
        end
    elseif k > 1;
        if size(pmod2_b2.param{k},1) < 40
            s = size(pmod2_b2.param{k},1);
            diff = 40 - s;
            pmod2_b2.param{k}(end+diff) = 0;
            pmod2_b2.poly{k}(end+diff) = 0;
...
for k = 1:2
    if k == 1;
        if size(pmod2_b3.param{k},1) < 20
            s = size(pmod2_b3.param{k},1);
            diff = 20 - s;
            pmod2_b3.param{k}(end+diff) = 0;
            pmod2_b3.poly{k}(end+diff) = 0;
        end
    elseif k > 1;
        if size(pmod2_b3.param{k},1) < 40
            s = size(pmod2_b3.param{k},1);
            diff = 40 - s;
            pmod2_b3.param{k}(end+diff) = 0;
            pmod2_b3.poly{k}(end+diff) = 0;
...
%block1
names = {'Pchoice','Lchoice','Tchoice','ConChoice','Icon_Choice','ConFB','IconFB','Pfbpos','Pfbneg','Lfbpos','Lfbneg','Tfbpos','Tfbneg','gameFBslow','sel','base_sel','Instructions'};
onsets = {gameP_disp1,gameL_disp1,gameT_disp1,congruent1_ons,incongruent1_ons,con_fb1_ons,icon_fb1_ons,gameP_fbp1,gameP_fbn1,gameL_fbp1,gameL_fbn1,gameT_fbp1,gameT_fbn1,fb_slow1,sel1,base1,trash1'};
durations = {dur_gameP_disp1,dur_gameL_disp1,dur_gameT_disp1,congruent1_dur,incongruent1_dur,con_fb1_dur,icon_fb1_dur,dur_gameP_fbp1,dur_gameP_fbn1,dur_gameL_fbp1,dur_gameL_fbn1,dur_gameT_fbp1,dur_gameT_fbn1,dur_fb_slow1,dur_sel1,dur_base1,dur_Instr};
pmod = pmod1;
pmodCondition = pmod2_b1;
if subnum < 10
    save(['sub00' int2str(subnum) '_block1'],'names','onsets','durations','pmod','pmodCondition')
elseif subnum >= 10
    save(['sub0' int2str(subnum) '_block1'],'names','onsets','durations','pmod','pmodCondition')
end

%block2
names = {'Pchoice','Lchoice','Tchoice','ConChoice','IconChoice','ConFB','IconFB','Pfbpos','Pfbneg','Lfbpos','Lfbneg','Tfbpos','Tfbneg','gameFBslow','sel','base_sel','Instructions'};
onsets = {gameP_disp2,gameL_disp2,gameT_disp2,congruent2_ons,incongruent2_ons,con_fb2_ons,icon_fb2_ons,gameP_fbp2,gameP_fbn2,gameL_fbp2,gameL_fbn2,gameT_fbp2,gameT_fbn2,fb_slow2,sel2,base2,trash2'};
durations = {dur_gameP_disp2,dur_gameL_disp2,dur_gameT_disp2,congruent2_dur,incongruent2_dur,con_fb2_dur,icon_fb2_dur,dur_gameP_fbp2,dur_gameP_fbn2,dur_gameL_fbp2,dur_gameL_fbn2,dur_gameT_fbp2,dur_gameT_fbn2,dur_fb_slow2,dur_sel2,dur_base2,dur_Instr};
pmod = pmod2;
pmodCondition = pmod2_b2;
if subnum < 10
    save(['sub00' int2str(subnum) '_block2'],'names','onsets','durations','pmod','pmodCondition')
elseif subnum >= 10
    save(['sub0' int2str(subnum) '_block2'],'names','onsets','durations','pmod','pmodCondition')
end

%block3
names = {'Pchoice','Lchoice','Tchoice','ConChoice','IconChoice','ConFB','IconFB','Pfbpos','Pfbneg','Lfbpos','Lfbneg','Tfbpos','Tfbneg','gameFBslow','sel','base_sel','Instructions'};
onsets = {gameP_disp3,gameL_disp3,gameT_disp3,congruent3_ons,incongruent3_ons,con_fb3_ons,icon_fb3_ons,gameP_fbp3,gameP_fbn3,gameL_fbp3,gameL_fbn3,gameT_fbp3,gameT_fbn3,fb_slow3,sel3,base3,trash3'};
durations = {dur_gameP_disp3,dur_gameL_disp3,dur_gameT_disp3,congruent3_dur,incongruent3_dur,con_fb3_dur,icon_fb3_dur,dur_gameP_fbp3,dur_gameP_fbn3,dur_gameL_fbp3,dur_gameL_fbn3,dur_gameT_fbp3,dur_gameT_fbn3,dur_fb_slow3,dur_sel3,dur_base3,dur_Instr};
pmod = pmod3;
pmodCondition = pmod2_b3;
if subnum < 10
    save(['sub00' int2str(subnum) '_block3'],'names','onsets','durations','pmod','pmodCondition')
elseif subnum >= 10
    save(['sub0' int2str(subnum) '_block3'],'names','onsets','durations','pmod','pmodCondition')
end   
