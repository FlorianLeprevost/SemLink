...
for k = 1:2
    if k == 1;
        if size(pmod2_b1(k).param{1},1) < 20
            s = size(pmod2_b1(k).param{1},1);
            diff = 20 - s;
            pmod2_b1(k).param{1}(end+diff) = 0;
            pmod2_b1(k).poly{1}(end+diff) = 0;
        end
    elseif k > 1;
        if size(pmod2_b1(k).param{1},1) < 40
            s = size(pmod2_b1(k).param{1},1);
            diff = 40 - s;
            pmod2_b1(k).param{1}(end+diff) = 0;
            pmod2_b1(k).poly{1}(end+diff) = 0;
...
for k = 1:2
    if k == 1;
        if size(pmod2_b2(k).param{1},1) < 20
            s = size(pmod2_b2(k).param{1},1);
            diff = 20 - s;
            pmod2_b2(k).param{1}(end+diff) = 0;
            pmod2_b2(k).poly{1}(end+diff) = 0;
        end
    elseif k > 1;
        if size(pmod2_b2(k).param{1},1) < 40
            s = size(pmod2_b2(k).param{1},1);
            diff = 40 - s;
            pmod2_b2(k).param{1}(end+diff) = 0;
            pmod2_b2(k).poly{1}(end+diff) = 0;
...
for k = 1:2
    if k == 1;
        if size(pmod2_b3(k).param{1},1) < 20
            s = size(pmod2_b3(k).param{1},1);
            diff = 20 - s;
            pmod2_b3(k).param{1}(end+diff) = 0;
            pmod2_b3(k).poly{1}(end+diff) = 0;
        end
    elseif k > 1;
        if size(pmod2_b3(k).param{1},1) < 40
            s = size(pmod2_b3(k).param{1},1);
            diff = 40 - s;
            pmod2_b3(k).param{1}(end+diff) = 0;
            pmod2_b3(k).poly{1}(end+diff) = 0;
...
pmod = pmod2_b1;
if subnum < 10
    save(['sub00' int2str(subnum) '_block1'],'names','onsets','durations','pmod')
elseif subnum >= 10
    save(['sub0' int2str(subnum) '_block1'],'names','onsets','durations','pmod')
end
...
pmod = pmod2_b2;
if subnum < 10
    save(['sub00' int2str(subnum) '_block2'],'names','onsets','durations','pmod')
elseif subnum >= 10
    save(['sub0' int2str(subnum) '_block2'],'names','onsets','durations','pmod')
end
...
pmod = pmod2_b3;
if subnum < 10
    save(['sub00' int2str(subnum) '_block3'],'names','onsets','durations','pmod')
elseif subnum >= 10
    save(['sub0' int2str(subnum) '_block3'],'names','onsets','durations','pmod')
end   
