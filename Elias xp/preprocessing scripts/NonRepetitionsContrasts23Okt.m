...
for subnum = 1:32;
    ...
    %% P-Selection v. LT Selection
    
    if mod(subnum,12) == 0 || mod(subnum,12) == 1 || mod(subnum,12) == 2 || mod(subnum,12) == 3;
        run1(SelReg) = 2; run1(SelBaseReg) = -2; 
        run2(SelReg) = -1; run2(SelBaseReg) = 1; 
        run3(SelReg) = -1; run3(SelBaseReg) = 1; 
        PvLTsel = [run1 run2 run3];
    elseif mod(subnum,12) == 4 || mod(subnum,12) == 5 || mod(subnum,12) == 8 || mod(subnum,12) == 9;
        run1(SelReg) = -1; run1(SelBaseReg) = 1;
        run2(SelReg) = 2; run2(SelBaseReg) = -2;
        run3(SelReg) = -1; run3(SelBaseReg) = 1;
        PvLTsel = [run1 run2 run3];
    elseif mod(subnum,12) == 6 || mod(subnum,12) == 7 || mod(subnum,12) == 10 || mod(subnum,12) == 11;
        run1(SelReg) = -1; run1(SelBaseReg) = 1;
        run2(SelReg) = -1; run2(SelBaseReg) = 1;
        run3(SelReg) = 2; run3(SelBaseReg) = -2;
        PvLTsel = [run1 run2 run3];
    end
    ...
    %% L-Selection v. PT Selection
    
    if mod(subnum,12) == 4 || mod(subnum,12) == 5 || mod(subnum,12) == 6 || mod(subnum,12) == 7;
        run1(SelReg) = 2; run1(SelBaseReg) = -2;
        run2(SelReg) = -1; run2(SelBaseReg) = 1;
        run3(SelReg) = -1; run3(SelBaseReg) = 1;
        LvPTsel = [run1 run2 run3];
    elseif mod(subnum,12) == 0 || mod(subnum,12) == 1 || mod(subnum,12) == 10 || mod(subnum,12) == 11;
        run1(SelReg) = -1; run1(SelBaseReg)= 1;
        run2(SelReg) = 2; run2(SelBaseReg) = -2;
        run3(SelReg) = -1; run3(SelBaseReg) = 1;
    ...
    %% T-Selection v. PL-Selection
    if mod(subnum,12) == 8 || mod(subnum,12) == 9 || mod(subnum,12) == 10 || mod(subnum,12) == 11;
        run1(SelReg) = 2; run1(SelBaseReg) = -2;
        run2(SelReg) = -1; run2(SelBaseReg) = 1;
        run3(SelReg) = -1; run3(SelBaseReg) = 1;
        TvPLsel = [run1 run2 run3];
    elseif mod(subnum,12) == 2 || mod(subnum,12) == 3 || mod(subnum,12) == 6 || mod(subnum,12) == 7;
        run1(SelReg) = -1; run1(SelBaseReg) = 1;
        run2(SelReg) = 2; run2(SelBaseReg) = -2;
        run3(SelReg) = -1; run3(SelBaseReg) = 1;
    ...
    %% Congruent v Incongruent Games per Dimension (NEGATIVE FEEDBACK)
    ...
    %% Feedback contrasts per dimension, irrelevant of congruency condition
    ...
    if subnum == 1;
        save('NonRepContrasts.mat','Psel','Lsel','Tsel','PvLTsel','LvPTsel','TvPLsel','P_CHOICE','L_CHOICE','T_CHOICE','P_FBpos','P_FBneg','L_FBpos','L_FBneg','T_FBpos','T_FBneg','sub1_allmods','sub1_conmod')
    elseif subnum == 13;
        save('NonRepContrasts.mat','Psel','Lsel','Tsel','PvLTsel','LvPTsel','TvPLsel','P_CHOICE','L_CHOICE','T_CHOICE','P_FBpos','P_FBneg','L_FBpos','L_FBneg','T_FBpos','T_FBneg','sub13_allmods','sub13_conmod')
    elseif subnum == 16;
        save('NonRepContrasts.mat','Psel','Lsel','Tsel','PvLTsel','LvPTsel','TvPLsel','P_CHOICE','L_CHOICE','T_CHOICE','P_FBpos','P_FBneg','L_FBpos','L_FBneg','T_FBpos','T_FBneg','sub16_allmods','sub16_conmod')
    elseif subnum == 31;
        save('NonRepContrasts.mat','Psel','Lsel','Tsel','PvLTsel','LvPTsel','TvPLsel','P_CHOICE','L_CHOICE','T_CHOICE','P_FBpos','P_FBneg','L_FBpos','L_FBneg','T_FBpos','T_FBneg','sub31_allmods','sub31_conmod')
    else;
        save('NonRepContrasts.mat','Psel','Lsel','Tsel','PvLTsel','LvPTsel','TvPLsel','P_CHOICE','L_CHOICE','T_CHOICE','P_FBpos','P_FBneg','L_FBpos','L_FBneg','T_FBpos','T_FBneg')
    end
