...
dir_epitmp = ['C:\Program Files\spm12\toolbox\OldNorm'];%directory of EPI template 
dir_tpm = ['C:\Program Files\spm12\tpm']; %directory of tissue probability map
fs              = filesep;
...
dir_MTohne      = 'MTohne'; 
dir_fm          = 'Fieldmap';
fm_phase        = '^0.*_14.*\.nii$';        %your fieldmap phase image (the single image) '^s\d.*-0016.*\.nii$'; 
fm_mag          = '^0.*_13_1.*\.nii$';    %your fieldmap magnitude image (the first of the two sequential images)'^s\d.*-0015.*.-01.*\.nii$';
sess_prfx       = 'run';
dummy_scans     = 0;
...
%     clear matlabbatch;     
    ...
    f   = spm_select('List', sourceDir, '^0.*\.nii$');
    files = [sourceDir fs f];
    matlabbatch{1}.spm.spatial.coreg.estimate.other = {files};       f = []; files = []; 
