...
part_files = ["subject-10.csv", "subject-15.csv", "subject-16.csv", "subject-17.csv"]
...
for i in range(4):
    plt.subplot(2,2,i+1)
    dats = list(avg[avg["phase"]==phases[i]]["correct"])
    sem = np.array(list(std[std["phase"]==phases[i]]["correct"]))/np.sqrt(len(part_files))
    plt.errorbar([1,2,3,4],dats,sem, ecolor="black", c="black",linewidth=0, marker="s",elinewidth=.5)
    plt.scatter([1,2,3,4],dats, marker="s", c="black",zorder=1)
    # plt.ylim([600,1100])
    plt.xticks([1,2,3,4], [0,1,0,1])
    ...
    if i==0 or i ==2:
        plt.ylabel("score")
plt.savefig('rewardspairedCORR.jpg')
